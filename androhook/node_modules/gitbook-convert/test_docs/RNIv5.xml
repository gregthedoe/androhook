...
<article xmlns="http://docbook.org/ns/docbook" version="5.0" xml:id="rni-es">
    <info>
                <mediaobject>
            <imageobject>
                <imagedata depth="1.0in" align="left" fileref="http://stylesheets.basistech.net/Logos/Basis-Technology.svg" format="SVG"/>
            </imageobject>
        </mediaobject>
     <mediaobject>
      <imageobject>
        ...
        <pubdate>_PUBMONTH_</pubdate>
        <releaseinfo>Version 7.13.0</releaseinfo>
        <legalnotice>
            <informaltable condition="standalone" border="" frame="none" colsep="0" rowsep="0">
                <tgroup cols="2">
                    <tbody>
                        <row>
                            <entry>Web:</entry>
                            <entry><link xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="http://www.basistech.com">http://www.basistech.com</link></entry>
                        </row>
                        <row>
                            <entry>Support:</entry>
            ...
            <para>Copyright  2015 Basis Technology Corporation. All rights
            ...
            <para>The software product described in this document is
                copyright  2015 Basis Technology Corporation.</para>
            <para>Lexical data used in this product are, unless otherwise
                stated, copyright  2015 Basis Technology Corporation.</para>
    ...
    <section xml:id="rni-es.intro">
        <title>Introduction</title>
        <para>RNI-Elasticsearch is an Elasticsearch<footnote><para>Copyright  2015 by Elasticsearch BV. Licensed under The Apache License Version 2.0.</para></footnote> plugin for building fuzzy name retrieval and matching applications for persons, locations,
     ...
     <section xml:id="rni-es.install">
        <title>Getting Started</title>
        <para>To use RNI-Elasticsearch, you need the RNI 7.13.0 Elasticsearch plugin, an RLP License, and Elasticsearch.</para>
        <orderedlist>
            <listitem>
                <para>If you do not already have it, install Elasticsearch.</para>
                <para>Download and unzip <link xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="https://www.elastic.co/downloads/elasticsearch">elasticsearch-1.x.y.zip</link>.</para>
            </listitem>
            <listitem>
                <para>Install the RNI-Elasticsearch plugin.</para>
                <para>Navigate to the <literal>elasticsearch-1.x.y</literal> root directory and run</para>
                <programlisting>bin/plugin --install rni-es --url file:///path/to/rni-es-7.13.0.zip</programlisting>
                <para>The RNI-Elasticsearch plugin is now in <literal>plugins/rni-es</literal>.</para>
            </listitem>
            <listitem>
                ...
                <para>This license must be in place before you can use RNI-Elasticsearch.</para>
            </listitem>
        </orderedlist>
        <para>To start the Elasticsearch server, run</para>
        <literallayout>bin/elasticsearch</literallayout>
    </section>
    <section xml:id="rni-es.usage">
        ...
        <para>The following snippets use the <link xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="http:/curl.haxx.se/">cURL</link>
        command-line tool to illustrate the Elasticsearch API for running the RNI-Elasticsearch plugin.</para>
    <section>
...
can minimize overhhead by turning off the "rni-multivalued" property, which is <literal>true</literal> by default.</para>
 ...
 <para>For the name fields, you can include individual properties in place of just a name string. Entity type is particularly useful.</para>
        <informaltable xml:id="rni-es.nameprops">
            <tgroup cols="3">
...
<literallayout>  "query" : {
    ...
    <para>The second part of the query uses Elasticsearch
        <link xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="http://www.elastic.co/guide/en/elasticsearch/reference/current/search-request-rescore.html">Rescoring</link>
    ...
    <tgroup cols="2">
      ...
      <tbody>
       ...
       <para>Scoring is commutative: the scores for two given names are always the same, regardless of which name is in the index and which name is in the query.</para>
         ...
         <section xml:id="rni-user-defined">
     <title>User-Configurable Features</title>
       ...
       <para>You can modify these files and add additional files in the same subdirectory to extend coverage to additional supported languages. You can also create
         ...
         <section xml:id="rni-sdk-user-defined-stop-patterns">
     ...
     <para>stopregexes_<emphasis>LANG</emphasis>[_<emphasis>TYPE</emphasis>].txt <footnote xml:id="rni-sdk-entity-overrides"><para>Include _<emphasis>TYPE</emphasis>, where <emphasis>TYPE</emphasis>
         ...
         <para>where <emphasis>LANG</emphasis> is the three-letter ISO 639-3 language code. Each row in the file, with the exception of rows that begin with
             <literal>#</literal>,<footnote xml:id="rkni-sdk.comments">
     ...
     <section xml:id="rni-sdk-user-defined-gray-list">
